...
def prob_fixation(s, p_iter, B=1):
    s_range = np.linspace(0, s, num = p_iter)
    g = np.vectorize(lambda s: np.log10(  (2*B / (s * (np.exp(2*B*s) -1)  )  ) \
        *  ((np.exp(2*B*s) + 1) * math.e  - expi(2*B*s) + np.log(2*B*s) \
        + np.exp(2*B*s) * (-expi(-2*B*s) + np.log(2*B*s))))  )
    return g(s_range)

line1 = prob_fixation(S, p_iter)
line2 = prob_fixation(S, p_iter, 2)
line3 = prob_fixation(S, p_iter, 3)
line4 = prob_fixation(S, p_iter, 4)
...
plt.plot(p, line1, color = 'b', linestyle = '-', label="B = 1")
plt.plot(p, line2, color = 'b',linestyle = '--', label="B = 2")
plt.plot(p, line3, color = 'b',linestyle = '-.', label="B = 3")
plt.plot(p, line4, color = 'b',linestyle = ':', label="B = 4")
