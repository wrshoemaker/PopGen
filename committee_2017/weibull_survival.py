...
mydir = os.path.expanduser("~/GitHub/PopGen/committee_2017/")
...
def half_life_hist():
    df = pd.read_csv(mydir + 'weibull_log_results.csv', sep = ',')
...
def irep_vs_half_life():
    df1 = pd.read_csv(mydir + 'weibull_log_results.csv', sep = ',')
    df1['Half_life_log'] = np.log10(df1.Half_life)
