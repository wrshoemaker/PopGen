...
def nasa_fig():
    ...
    ax1.plot(x_axis1, norm.pdf(x_axis1,0,2), 'k', linewidth = 2,  label="Ancestor")
    ax1.plot(x_axis2, norm.pdf(x_axis2,15,2), 'b:',linewidth = 2, label='Spore forming')
    ax1.plot(x_axis3, norm.pdf(x_axis3,25,2), 'r--', linewidth = 2,label="Non-spore forming")
    ax1.set_xlim([-15,47])
    plt.xlabel('Fitness (i.e., growth rate)', fontsize = 14)
    ...
    ax1.text(-0.8, 0.22, r'$T_{0}$', fontsize=11, fontweight='bold', color= 'k')
    ax1.text(14.2, 0.22, r'$T_{1}$', fontsize=11, fontweight='bold', color = 'b')
    ax1.text(24.2, 0.22, r'$T_{1}$', fontsize=11, fontweight='bold', color = 'r')
    ...
    ax2.plot(x_eval, kde2(x_eval), 'b:', linewidth = 2, label='Spore forming', alpha = 0.8)
    ...
    for item in zip_x_y:
        if (item[0] < 0) and (item[1] > 0.08):
            ...
            new_y.append(item[1])
    ax2.plot(x_eval, new_y, 'r--', linewidth = 2, label="Non-spore forming", alpha = 0.8)
    ax2.text(-5.67, 0.22, "b)", fontsize=12, fontweight='bold')
