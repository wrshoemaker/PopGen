...
def viterbi(IN, lengthParams, emissionParams, stateParams, initialParams):
    ...
    returnObject = (likelihood, zip(list(IN), stateList))
    return returnObject
...
if __name__ == "__main__":
    ...
    runAnalysis = viterbi(INsequence, lengthParams, emissionParams, stateParams,initialParams)
    OUT =  open(OUT, 'w+')
    print>> OUT, runAnalysis[0]
    for x in runAnalysis[1]:
        #orf_name = "ORF" + str(x)
        print>> OUT, x[0], x[1]
